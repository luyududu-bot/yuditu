...
- 本地目标解析器只从已显影地点中读取语义、时间约束、代表性与空间距离，生成带逐站理由的可解释路线。
- 路线生成后自动进入专注模式，隐藏无关地点、聚焦路线范围，并显示顺序节点与 SVG 曲线路径。
- 设施图标使用类别视觉锚点校准，编号、点击区域和路线端点共享同一视觉中心。
- 地点详情中的对象 action 可真实执行地图聚焦、加入路线、写入手账与设置起点。
...
未来平台级 Agent 可以读取这些对象，理解用户目标，并调用受控 action 聚焦地点、生成路线或创建探索记录。当前版本已提供对象查询、action 权限过滤、执行分发与数据关系校验的最小闭环。
...
首版使用零依赖静态 Web 技术实现，便于快速部署与稳定演示。核心数据与目标路线逻辑位于 `src/data.mjs` 和 `src/core.mjs`，对象 action 与校验位于 `src/object-actions.mjs`；用户解锁进度与手账排版使用 `localStorage` 保存，手账通过 Canvas 导出为 PNG。
